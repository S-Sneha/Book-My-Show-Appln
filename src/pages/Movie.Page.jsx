...
useEffect(() => {
  ...
  const getMovieDetail = await axios.get(`/movie/${id}`);
  setMovie(getMovieDetail.data);
  };
  requestMovie();
},[id]);
...
const settings = {
  ...
  return (
    ...
    <PosterSlider config={settings} title="BMS EXCLUSIVE" posters={recommendedMovies} isDark={false}/>
  </div> 
  <div className="my-8">
        <hr/>
   </div>  
